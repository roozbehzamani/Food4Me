var _scq = _scq || [];
_scq.push(['setMerchantId', '192FE89797F4EA77']);
...
;var ScarabModule = function(win, config) {
  ...
  Transaction.prototype = {eventHandlers:{addView:function(viewedItem) {
    ...
    var clickedItem;
    if (developMode) {
      clickedItem = getClickedItemCookie(STRINGS.VIEWCOOKIE);
      if (clickedItem) {
      ...
      clickedItem = findInCookie(STRINGS.VIEWCOOKIE, viewedItem);
      if (clickedItem) {
        viewedItem.merge(clickedItem);
